...
def test_client(node, hardware, map, mission):
    """
    Tests the mission goal manager action server.

    This function creates a MissionPlannerGoal message from the given mission data with op set to 2. It sends this message to the "harpia/mission_goal_manager" action server and publishes it to the "/harpia/mission" topic while the action server state is less than 2. It waits for the action server to start up before sending the goal and continues to publish the goal while the server is active or pending. If ROS is shutting down, it logs a warning and breaks the loop.

    Parameters
    ----------
    node : Node
        The ROS node to use for creating the ActionClient and Publisher.
    hardware : dict
        The hardware dictionary. It should contain the hardware configuration for the mission.
    map : dict
        The map dictionary. It should contain the map configuration for the mission.
    mission : dict
        The mission dictionary. It should contain the mission configuration.

    Returns
    -------
    MissionPlannerResult
        The result of executing the action. The type is assumed based on the function's usage.
    """
    ...
    pub = node.create_publisher(Mission, "/harpia/mission", QoSProfile(depth=10))
...
if __name__ == '__main__':
    ...
    try:
        node = Node('test_client1')
        client = ActionClient(node, MissionPlanner, 'harpia/mission_goal_manager')
        result = test_client(client, hardware, map, mission)
