...
{
    node_ = rclcpp::Node::make_shared("rpharpia_executor");

    mission_fault_client_ = node_->create_client<interfaces::srv::MissionFaultMitigation>("harpia/mission_fault_mitigation");
    waypoint_push_client_ = node_->create_client<mavros_msgs::srv::WaypointPush>("mavros/mission/push");
    waypoint_clear_client_ = node_->create_client<mavros_msgs::srv::WaypointClear>("mavros/mission/clear");
    ...
    if (client->wait_for_service(std::chrono::seconds(10)))
    {
        ...
        if (rclcpp::spin_until_future_complete(node_, result) == rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            if (replan != 1)
            {
                ...
                RCLCPP_INFO(node_->get_logger(), "%s", msg->action.c_str());
                ...
                if (found != std::string::npos)
                {
                    ...
                    RCLCPP_INFO(node_->get_logger(), "go_to %s -> %s", from.name.c_str(), to.name.c_str());
                    ...
                    RCLCPP_INFO(node_->get_logger(), "GEO GeoPoint %f %f %f -> %f %f %f", r_from.geo.latitude, r_from.geo.longitude, r_from.geo.altitude, r_to.geo.latitude, r_to.geo.longitude, r_to.geo.altitude);
                    ...
                    RCLCPP_INFO(node_->get_logger(), "KCL: (%s) HarpiaExecutor Action completing.", msg->action.c_str());
                }
                // Continue com outros casos de `msg->action` como "pulverize_region", "take_image", etc.
            }
            else
            {
                RCLCPP_INFO(node_->get_logger(), "NEED TO REPLAN");
            }
        }
        else
        {
            RCLCPP_ERROR(node_->get_logger(), "Failed to call service harpia/mission_fault_mitigation");
        }
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(plansys2::RPHarpiaExecutor)
